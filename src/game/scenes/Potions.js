...
//         this.timer.start();
//         // this.player1 = new Player(this, 150, 300);
//         // this.player2 = new Player(this, 1100, 300);
...
//         // --- INSIDE create(), after background ----

//         // 🔥 WAND TARGET CURSOR
//         this.cursor = this.add.graphics();
//         this.cursor.lineStyle(3, 0xBDB9FA);
//         this.cursor.strokeCircle(0, 0, 20);
...
//                 this.particles.explode(60, pointer.x, pointer.y);
//                 this.lastShot = now;
//             }
//         });
...
//         // Borders
//         this.p1Border = this.add.graphics().setDepth(5);
//         this.p2Border = this.add.graphics().setDepth(5);
...
//         // Potion icons
//         this.potion1 = this.add.image(400, 300, 'glow').setScale(0.1);
//         this.potion2 = this.add.image(550, 300, 'grow').setScale(0.1);
//         this.potion3 = this.add.image(700, 300, 'trace').setScale(0.1);
//         this.potion4 = this.add.image(850, 300, 'colour').setScale(0.1);
...
//         // Players appear
//         this.players = [];
//         const p1 = new Player(this, 150, 300).setScale(0.25);
//         this.players.push(p1);

//         if (PlayersNum.players === 3) {
//             const p2 = new Player(this, 1100, 300).setScale(0.25);
//             this.players.push(p2);
//         }
...
//         // 🔥 TIMER COMPLETES → randomize and start game
//         this.timer.onComplete = () => {
//             console.log("Timer finished → random potions applied!");
...
//         // // 🔥 CLICK → skip timer and start immediately
//         // this.input.once("pointerdown", () => {
//         //     ChosenPowers.p1 = this.selected1;
//         //     ChosenPowers.p2 = PlayersNum.players === 3 ? this.selected2 : null;
...
//     updateBorders() {
//         this.p1Border.clear().lineStyle(6, 0x5DA9FF, 1);
//         this.p2Border.clear().lineStyle(6, 0xFFE45D, 1);
...
export class Potions extends Scene {
    ...
    create() {
        ...
        // BACKGROUND
        // -------------------------------
        this.bg = this.add.image(0, 0, 'cabin').setOrigin(0);
        this.bg.displayWidth = this.scale.width;
        this.bg.displayHeight = this.scale.height;
        this.bg.setDepth(-10);
        ...
        this.input.on("pointerdown", (p) => {
            if (this.time.now > this.lastShot + this.shootDelay) {
                this.particles.explode(60, p.x, p.y);
        ...
        // POTIONS
        //-------------------------------
            this.p1Border = this.add.graphics().setDepth(10);
        this.p2Border = this.add.graphics().setDepth(10);
    ...
    update(time, delta) {
        ...
        const joy1 = Input.joy1;
        const joy2 = Input.joy2;
        ...
        // -------------------------------
        if (!this.p1Locked) {
            if (joy1 === "left" && !this.p1LeftHeld) {
                this.selected1 = Math.max(0, this.selected1 - 1);
                this.p1LeftHeld = true;
                this.previewPower(this.player1, this.selected1);
                this.updateBorders();
            }
            if (joy1 !== "left") this.p1LeftHeld = false;

            if (joy1 === "right" && !this.p1RightHeld) {
                this.selected1 = Math.min(3, this.selected1 + 1);
                this.p1RightHeld = true;
                this.previewPower(this.player1, this.selected1);
                this.updateBorders();
            }
            if (joy1 !== "right") this.p1RightHeld = false;
        }

        // -------------------------------
        // PLAYER 1 LOCK WITH BUTTON
        // -------------------------------
        if (!this.p1Locked && btn1 === 1 && this.prevBtn1 === 0) {
            this.p1Locked = true;
            ChosenPowers.p1 = this.selected1;
            console.log("Player 1 locked →", this.selected1);
        }
        ...
        // -------------------------------
        if (PlayersNum.players === 3 && !this.p2Locked) {
            if (joy2 === "left" && !this.p2LeftHeld) {
                this.selected2 = Math.max(0, this.selected2 - 1);
                this.p2LeftHeld = true;
                this.previewPower(this.player2, this.selected2);
                this.updateBorders();
            }
            if (joy2 !== "left") this.p2LeftHeld = false;

            if (joy2 === "right" && !this.p2RightHeld) {
                this.selected2 = Math.min(3, this.selected2 + 1);
                this.p2RightHeld = true;
                this.previewPower(this.player2, this.selected2);
                this.updateBorders();
            }
            if (joy2 !== "right") this.p2RightHeld = false;
        }

        // -------------------------------
        // PLAYER 2 LOCK WITH BUTTON
        // -------------------------------
        if (!this.p2Locked && btn2 === 1 && this.prevBtn2 === 0) {
            this.p2Locked = true;
            ChosenPowers.p2 = this.selected2;
            console.log("Player 2 locked →", this.selected2);
        }

        // -------------------------------
        // IF BOTH PLAYERS LOCKED → GO NEXT SCENE
        // -------------------------------
        if (this.p1Locked && this.p2Locked) {
            console.log("Both locked → starting AnimationTwo...");
            this.time.delayedCall(300, () => {
                this.scene.start("AnimationTwo");
            });
        }
    ...
    //     // Player objects animate
    //     this.player1.update(time, delta);
    //     if (this.player2) this.player2.update(time, delta);

    //     // ---- PLAYER 1 SELECT LEFT / RIGHT
    //     if (joy1 === "left" && !this.p1LeftHeld) {
    //         this.selected1 = Math.max(0, this.selected1 - 1);
    //         this.p1LeftHeld = true;
    //         this.previewPower(this.player1, this.selected1);
    ...
    updateBorders() {
        this.p1Border.clear().lineStyle(6, 0x5DA9FF);
        const p1 = this.potions[this.selected1];
        this.p1Border.strokeRect(
            p1.x - p1.displayWidth / 2 - 10,
            p1.y - p1.displayHeight / 2 - 10,
        ...
        if (PlayersNum.players === 3) {
            this.p2Border.clear().lineStyle(6, 0xFFE45D);
            const p2 = this.potions[this.selected2];
            this.p2Border.strokeRect(
                p2.x - p2.displayWidth / 2 - 10,
                p2.y - p2.displayHeight / 2 - 10,
